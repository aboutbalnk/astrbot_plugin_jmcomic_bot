# AstrBot JMComic Bot

在 AstrBot 中使用 JMComic-Crawler-Python 查询、智能搜索、下载 JM 内容，并导出 PDF 发送到聊天。

## 功能

- `jm 查询 <id>` 查询本子信息。
- `jm 搜索 <关键词>` 搜索本子并展示结果。
- `jm <id> 同作者` 根据参考本子查找同作者作品。
- `jm <id> 同风格` 根据参考本子的作品/标签查找相似风格作品。
- `jm <id> 同作者 同风格` 根据参考本子混合查找同作者和相似风格作品。
- `jm <id>` 直接下载、压缩为 PDF 并发送。
- 支持自然语言搜索，群聊可配置为只有 @ 机器人时触发。
- 支持 LLM 生成智能检索计划，例如按观看/喜欢排序、作者检索、短篇约束、排除连载等。
- 支持 PDF 压缩、PDF 随机数字密码加密、发送后自动清理本地文件。
- 支持 AstrBot 统一会话 ID，把 PDF 额外发送到其他协议/会话。
- 针对 OneBot/NapCat 上传超时做了兼容：上传接口超时时不直接提示失败，并延迟删除本地 PDF。

## 安装

在 AstrBot 插件目录中安装：

```bash
git clone https://github.com/aboutbalnk/astrbot_plugin_jmcomic_bot.git
```

然后在 AstrBot WebUI 中安装依赖或在容器内执行：

```bash
pip install -r requirements.txt
```

如果使用 Docker 部署 AstrBot，请在 AstrBot 容器内安装依赖。

## LLM 前提

智能搜索依赖 AstrBot 已经配置并启用了可用的 LLM Provider。插件会调用 AstrBot 当前会话的模型生成搜索计划。

如果没有配置 LLM，或者模型调用失败，插件会自动降级为规则搜索；基础功能仍可使用，包括：

```text
jm 查询 <id>
jm 搜索 <关键词>
jm <id>
```

## LLM 对话示例

开启自然语言和 LLM 后，可以直接用接近日常聊天的方式让插件生成搜索计划。群聊中如果开启了 `group_natural_language_require_at`，这些自然语言请求需要 @ 机器人；`jm` 开头的明确指令不受影响。

```text
用户：我想看某个主题，并且要看的人多一点
插件：按主题搜索，并倾向按观看数排序。

用户：帮我找作者 ABC 的作品，按浏览量排序，前 5 个
插件：使用作者检索，扩大候选池，并只展示前 5 个结果。

用户：找画风类似某作品的短篇，不要超过 3 章
插件：按作品名或相似风格检索，并过滤章节数超过 3 的结果。

用户：搜索标签 A 和标签 B，都要包含，按喜欢排序
插件：保留组合关键词或高级搜索语法，并按喜欢数排序。

用户：我想看 JM123456 作者的其他作品
插件：先查询 JM123456 的详情，再按作者搜索，并排除 JM123456 本身。

用户：1447462 同风格，按喜欢排序，前 10 个
插件：把 1447462 当作参考本子，使用作品/标签生成相似风格搜索，并按喜欢数排序。

用户：下载第一个
插件：下载最近一次搜索结果中的第一个条目，导出 PDF 后发送。
```

LLM 只负责把用户的话转换成搜索参数，不会直接访问 JM，也不会自动批量下载搜索结果。即使用户说“下载前 5 个”，插件也只会展示搜索结果；需要下载时再发送 `jm <id>` 或 `jm 第一个`。

## 智能搜索如何工作

用户发送自然语言搜索时，插件不会让 LLM 直接访问 JM，也不会让 LLM 下载内容。LLM 只负责把用户的话整理成搜索方案，例如关键词、作者搜索、标签搜索、按观看排序、按喜欢排序等。

搜索方案不是固定 6 条。关键词明确时，LLM 通常只会给 1 条高质量检索；只有用户描述比较模糊，或者需要从作者、作品、标签、角色等不同角度尝试时，才会生成多组检索。

插件拿到搜索方案后会按下面的流程处理：

1. 最多生成 6 组搜索方案，但不会为了凑数量强行生成。
2. 逐组调用 JM 搜索，插件会按查询类型自动决定每组搜索几页。
3. 把不同搜索方案得到的结果按 JM ID 去重合并。
4. 对候选结果补充详情，例如观看数、喜欢数、页数、章节数和标签。
5. 根据配置过滤结果，例如过滤连载标签、过滤页数或章节数过多的内容。
6. 如果用户要求“热门”“观看多”，就按观看数排序；如果要求“喜欢多”“点赞多”，就按喜欢数排序。
7. 最后只展示配置允许的前几条结果。

相关配置：

- `search_plan_limit`: 最多执行多少组搜索方案，默认 6，范围 1-20。
- `search_result_limit`: 最终最多展示多少条结果，默认 10，范围 1-50。
- `search_candidate_limit`: 智能搜索最多保留多少个候选，默认 30，范围为 `search_result_limit` 到 200。
- `search_page_limit`: 每组搜索方案最多检索多少页，默认 3，范围 1-10。插件会按查询自动决定实际页数。
- `max_search_page_count`: 过滤超过多少页或章节的内容，默认 100，范围 1-1000。
- `filter_serial_tags`: 是否过滤连载/連載标签。

如果这些数值配置超出范围，插件会在启动时自动修正到允许范围，不会中断启动。

`search_page_limit` 是上限，不代表每个搜索方案都会翻到这个页数。作者检索、按观看/喜欢排序、带页数/章节过滤的搜索会倾向于多取几页；如果 LLM 生成了多组不同关键词，插件会尽量减少每组翻页，避免重复请求。

JM 的排序只在具体搜索关键词下生效。比如“作者 XXX，按浏览量排序”这种请求，关键词本身很明确，重复生成多组搜索意义不大，更适合在同一个作者搜索下多翻几页，再从候选里按浏览量排序。调大 `search_page_limit` 可以扩大候选池，代价是搜索更慢、请求更多。

## 命令

```text
jm 查询 <id>
jm 搜索 <关键词>
jm <id> 同作者
我想看 JM<id> 作者的其他作品
jm <id> 同风格
jm <id> 同作者 同风格
jm <id>
jm 章节 <photo_id>
jm 第一个
```

`jm 第一个` 会下载最近一次搜索结果中的第一个条目。

`jm <id> 同作者`、`我想看 JM<id> 作者的其他作品`、`jm <id> 同风格` 和 `jm <id> 同作者 同风格` 不会下载参考本子。插件会先查询参考本子的详情，再用作者、作品或标签生成搜索计划，并从结果中排除参考本子本身。后面也可以继续加排序和数量要求，例如“按喜欢排序，前 10 个”。

## 关键配置

- `download_dir`: JM 图片临时下载目录。
- `pdf_dir`: PDF 输出目录。
- `compress_temp_dir`: 压缩 PDF 时的临时目录。
- `runtime_dir`: 插件生成 jmcomic 运行时配置文件的目录。
- `max_send_mb`: PDF 目标大小。
- `pdf_compress_profiles`: PDF 压缩档位，格式如 `1600:68,1280:58,1000:48`。
- `encrypt_pdf`: 是否发送前加密 PDF。
- `pdf_password_digits`: 随机数字密码位数。
- `cleanup_download_images`: 生成 PDF 后是否清理图片目录。
- `send_to_current_session`: 是否发送回触发命令的当前会话。
- `target_sessions`: 额外发送目标，使用 AstrBot 统一会话 ID。
- `notify_password_to_target_sessions`: 是否向额外目标发送 PDF 密码。
- `enable_natural_language`: 是否开启自然语言搜索/下载。
- `group_natural_language_require_at`: 群聊自然语言是否需要 @ 机器人。
- `proxy`: JM 搜索和下载请求使用的代理。

`enable_natural_language` 和更复杂的智能检索效果依赖 AstrBot LLM Provider。未配置 LLM 时，插件会使用内置规则做基础搜索。

代理配置示例：

```text
proxy: http://172.23.0.1:7890
```

Docker 部署时，`127.0.0.1` 指的是 AstrBot 容器内部，不是宿主机。若代理运行在宿主机 Clash/Mihomo 上，通常应填写 Docker 网关地址，例如 `http://172.23.0.1:7890`。留空时使用 jmcomic 默认/系统代理；填写 `none`、`off` 或 `关闭` 可显式禁用代理。

额外发送目标示例：

```text
default:GroupMessage:123456789
default:FriendMessage:123456789
group:123456789
friend:123456789
```

完整格式推荐使用 `平台ID:消息类型:会话ID`，例如 `default:GroupMessage:123456789`。`group:` 和 `friend:` 是当前平台下的简写。

## 目录配置

默认目录按 AstrBot Docker 容器内路径设置：

```text
/AstrBot/data/jmcomic
/AstrBot/data/jmcomic_pdf
/AstrBot/data/jmcomic_compress
/AstrBot/data/plugin_data/astrbot_plugin_jmcomic
```

如果你使用 Docker 部署 AstrBot，通常保持默认即可。路径是容器内路径，不是宿主机路径；需要查看文件时，请确认 Docker volume 映射到宿主机的位置。

如果你不是 Docker 部署，或者本机没有 `/AstrBot/data` 写入权限，请在 AstrBot WebUI 中把这些配置改成 AstrBot 进程可写目录，例如：

```text
download_dir: ./data/jmcomic
pdf_dir: ./data/jmcomic_pdf
compress_temp_dir: ./data/jmcomic_compress
runtime_dir: ./data/plugin_data/astrbot_plugin_jmcomic
```

## 注意

- 本插件会下载并发送 NSFW 内容，请遵守你所在地区法律法规和聊天平台规则。
- PDF 文件较大时，OneBot/NapCat 可能出现 WebSocket API timeout。该情况不一定代表文件发送失败，插件会延迟清理本地 PDF。
- 额外目标会话是否支持文件组件取决于对应 AstrBot 平台适配器。

## 第三方项目

本插件使用并随包附带 `jmcomic` 模块，来源于 [JMComic-Crawler-Python](https://github.com/hect0x7/JMComic-Crawler-Python)，其许可证为 MIT。详见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。

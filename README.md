...
用户发送自然语言搜索时，插件不会让 LLM 直接访问 JM，也不会让 LLM 下载内容。LLM 只负责把用户的话整理成几个搜索方案，例如关键词、作者搜索、标签搜索、按观看排序、按喜欢排序等。
...
`search_page_limit` 是上限，不代表每个搜索方案都会翻到这个页数。作者检索、按观看/喜欢排序、带页数/章节过滤的搜索会倾向于多取几页；如果 LLM 生成了多组不同关键词，插件会尽量减少每组翻页，避免重复请求。调大它可以扩大候选池，代价是搜索更慢、请求更多。
